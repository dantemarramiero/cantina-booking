...
function createEventBus(db, { log = console.error } = {}) {
  ...
  // version: versione dello schema del payload (un cambiamento incompatibile è una versione nuova);
  // actor: chi ha fatto l'azione, come testo leggibile.
  function emit(type, { sourceTable = null, sourceId = null, payload = {}, version = 1, actor = null } = {}) {
    return Number(db.prepare('INSERT INTO domain_events (type, source_table, source_id, payload, created_at, schema_version, actor) VALUES (?, ?, ?, ?, ?, ?, ?)')
      .run(type, sourceTable, sourceId, JSON.stringify(payload), now(), version, actor).lastInsertRowid);
  }
