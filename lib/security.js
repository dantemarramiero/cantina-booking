...
const API_WORKSPACES = {
  ...
  // Produzione e Magazzino. Le API del modulo Produzione (prd) controllano in più le capacità del ruolo.
  production: only('produzione'),
  produzione: only('produzione'),
  prd: only('produzione'),
  warehouse: only('magazzino'),
  magazzino: only('magazzino'),
  // Registro valorizzato: lo leggono anche Commerciale, Enoturismo e Finance; lo scrive il Magazzino.
  stock: { read: ['magazzino', 'commerciale', 'enoturismo', 'finance'], write: ['magazzino'] },
  // Degustazioni proposte dal check-in: le conferma chi fa le visite o il magazzino.
  'stock/tastings': { read: ['magazzino', 'enoturismo'], write: ['magazzino', 'enoturismo'] },
  // CRM
  ...
  suppliers: { read: ['crm', 'magazzino', 'produzione'], write: ['crm'] }, // Produzione: tonnellerie, laboratori, terzisti
  people: only('crm'),
  crm: only('crm', 'commerciale'),
  // People (risorse umane) e Finance
  hr: only('people'),
  'hr/directory': { read: ANY, write: ['people'] }, // livello "base": nome, ruolo, sede, contatti aziendali
  // Assenze: ognuno chiede le proprie e il responsabile decide anche senza il workspace People;
  // chi vede e chi decide lo controlla il modulo (sé stesso, collaboratori, livello "personale").
  'hr/absences': { read: ANY, write: ANY },
  'hr/absence-types': { read: ANY, write: ['people'] },
  // Presenze: ognuno registra le proprie ore, il caposquadra quelle della squadra; controlli nel modulo.
  'hr/timesheet': { read: ANY, write: ANY },
  // Self-service: il proprio fascicolo e le richieste di modifica; i propri documenti si scaricano
  // (il download controlla livello del tipo e visibilità al dipendente).
  'hr/me': { read: ANY, write: ANY },
  'hr/documents': { read: ANY, write: ['people'] },
  finance: only('finance'),
  'finance/cost-centers': { read: ['finance', 'people'], write: ['finance'] }, // People sceglie il centro dei dipendenti
};

// Livelli di riservatezza dei dati HR. "base" ce l'ha chiunque; gli altri si danno per ruolo.
const ACCESS_LEVELS = ['base', 'personale', 'retributivo', 'sanitario', 'disciplinare'];
// Capacità dentro Produzione (chi approva, chi esegue, chi registra in vigneto…). "sola_lettura" non scrive mai.
const PRD_CAPABILITIES = ['enologo', 'cantiniere', 'capo_squadra', 'agronomo', 'responsabile_qualita', 'sola_lettura'];

function apiGroup(pathname) {
  const m = String(pathname).match(/^\/api\/admin\/([a-z0-9-]+)/);
  return m ? m[1] : null;
}
// La regola più specifica vince: prima "gruppo/sottogruppo", poi "gruppo".
function ruleFor(pathname) {
  const m = String(pathname).match(/^\/api\/admin\/([a-z0-9-]+)(?:\/([a-z0-9-]+))?/);
  if (!m) return undefined;
  return (m[2] && API_WORKSPACES[`${m[1]}/${m[2]}`]) || API_WORKSPACES[m[1]];
}
...
module.exports = {
  ...
  API_WORKSPACES, ACCESS_LEVELS, PRD_CAPABILITIES, SESSION_IDLE_MS, SESSION_MAX_MS,
};
