...
function inTransaction(db, mod, fn) {
  ...
  try {
    fn();
    if (fkOff) {
      // Si controllano solo i collegamenti delle tabelle ricostruite (rebuilds): eventuali riferimenti
      // orfani già presenti altrove nei dati storici non devono impedire l'avvio.
      const touched = mod.rebuilds || [];
      const broken = db.prepare('PRAGMA foreign_key_check').all().filter(v => !touched.length || touched.includes(v.parent) || touched.includes(v.table));
